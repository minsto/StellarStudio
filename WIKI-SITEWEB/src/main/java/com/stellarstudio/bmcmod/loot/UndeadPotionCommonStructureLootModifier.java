...
public final class UndeadPotionCommonStructureLootModifier extends LootModifier {
    public static final MapCodec<UndeadPotionCommonStructureLootModifier> CODEC = RecordCodecBuilder.mapCodec(inst ->
            LootModifier.codecStart(inst).apply(inst, UndeadPotionCommonStructureLootModifier::new));

    private static final float CHANCE = 0.13F;

    public UndeadPotionCommonStructureLootModifier(LootItemCondition[] conditions) {
        super(conditions);
    }
    ...
    protected ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
        ...
        int tier = 1 + context.getRandom().nextInt(2); // 1..2
